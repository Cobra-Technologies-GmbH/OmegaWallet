...
// Providers
import { ClipboardProvider, ElectronProvider, IncomingDataProvider, Logger, PlatformProvider } from '..';
describe('ClipboardProvider', () => {
    let clipboardProvider;
    let clipboard;
    let electronProvider;
    ...
    describe('Mobile: ', () => {
        ...
        describe('copy', () => {
            it('should copy data', () => {
                const copySpy = spyOn(clipboard, 'copy').and.returnValue(Promise.resolve());
                try {
        ...
        describe('clearClipboardIfValidData', () => {
            it('should clear clipboard if data type is valid', fakeAsync(() => {
                spyOn(clipboard, 'paste').and.returnValue(Promise.resolve('1CVuVALD6Zo7ms24n3iUXv162kvUzsHr69')); // BitcoinAddress
                spyOn(incomingDataProvider, 'parseData').and.returnValue({
                    type: 'BitcoinAddress'
                });
                spyOn(clipboard, 'copy').and.returnValue(Promise.resolve());
                const typeArray = ['BitcoinAddress'];
    ...
    describe('Desktop: ', () => {
        ...
        describe('getData', () => {
            it('should get copied data', () => {
                spyOn(electronProvider, 'readFromClipboard').and.returnValue(Promise.resolve('data1'));
                clipboardProvider
                    .getData()
                    .then(data => {
                    expect(data).toEqual('data1');
        ...
        describe('getValidData', () => {
            it('should return copied value (1/2)', () => {
                ...
                spyOn(electronProvider, 'readFromClipboard').and.returnValue(Promise.resolve(data.data));
                clipboardProvider.getValidData(data.coin).then(res => {
                    expect(res).toEqual(data.data);
                });
            });
            ...
            it('should return empty if coin is not defined', () => {
                const address = 'mq8Hc2XwYqXw4sPTc8i7wPx9iJzTFTBWbQ';
                spyOn(electronProvider, 'readFromClipboard').and.returnValue(Promise.resolve(address));
                spyOn(clipboardProvider, 'getValidData').and.returnValue(Promise.resolve());
            });
        });
        describe('copy', () => {
        ...
        describe('clearClipboardIfValidData', () => {
            it('should clear clipboard if data type is valid', fakeAsync(() => {
                spyOn(electronProvider, 'readFromClipboard').and.returnValue(Promise.resolve('1CVuVALD6Zo7ms24n3iUXv162kvUzsHr69')); // BitcoinAddress
                spyOn(incomingDataProvider, 'parseData').and.returnValue({
                    type: 'BitcoinAddress'
                });
