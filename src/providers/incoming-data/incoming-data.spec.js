...
describe('Provider: Incoming Data Provider', () => {
    ...
    beforeEach(() => {
        ...
        actionSheetSpy = spyOn(testBed.get(ActionSheetProvider), 'createIncomingDataMenu').and.returnValue({
            present() { },
            onDidDismiss() { }
        });
    });
    describe('Function: SCANNER Redir', () => {
        it('Should handle plain text', () => {
            let data = [
                ...
                'cNJFgo1driFnPcBdBX8BrJrpxchBW XwXCvNH5SoSkdcF6JXXwHMm',
                'tprv8ZgxMBicQKsPcsbCVeqqF1KVdH7gwDJbxbzpCxDUsoXHdb6SnTPY xdwSAKDC6KKJzv7khnNWRAJQsRA8BBQyiSfYnRt6zuu4vZQGKjeW4YF',
                'Jason was here'
            ];
            data.forEach(element => {
                expect(incomingDataProvider.redir(element, { activePage: 'ScanPage' })).toBe(true);
        ...
        it('Should handle Join Wallet', () => {
            ...
            expect(incomingDataProvider.redir(data, { activePage: 'ScanPage' })).toBe(true);
        ...
        it('Should handle QR Code Export feature', () => {
            ...
            data.forEach(element => {
                ...
                expect(incomingDataProvider.redir(element, { activePage: 'ScanPage' })).toBe(true);
                expect(loggerSpy).toHaveBeenCalledWith('Incoming-data (redirect): QR code export feature');
                expect(eventsSpy).toHaveBeenCalledWith('IncomingDataRedir', nextView);
            });
        });
        it('Should handle BitPay Invoices with different coins', () => {
            ...
            data.forEach(element => {
                expect(incomingDataProvider.redir(element, { activePage: 'ScanPage' })).toBe(true);
                expect(loggerSpy).toHaveBeenCalledWith('Incoming-data: Payment Protocol with non-backwards-compatible request');
            });
        });
        it('Should parse valid BitPay Invoice Url if selectedtransactionCurrency exists with different coins', fakeAsync(() => {
            ...
            const getPayProDetailsSpy = spyOn(payproProvider, 'getPayProDetails').and.returnValue(Promise.resolve(payProDetails));
            ...
            data.forEach(element => {
                ...
                expect(loggerSpy).toHaveBeenCalledWith('Incoming-data: Handling bitpay invoice');
                tick();
                const stateParams = {
        ...
        it('Should handle Bitcoin cash Copay/BitPay format and CashAddr format plain Address', () => {
            ...
            data.forEach(element => {
                expect(incomingDataProvider.redir(element, { activePage: 'ScanPage' })).toBe(true);
                expect(loggerSpy).toHaveBeenCalledWith('Incoming-data: Bitcoin Cash plain address');
                expect(actionSheetSpy).toHaveBeenCalledWith({
                    data: {
                        data: element,
                        type: 'bitcoinAddress',
        ...
        it('Should handle ETH plain Address', () => {
            ...
            data.forEach(element => {
                expect(incomingDataProvider.redir(element, { activePage: 'ScanPage' })).toBe(true);
                expect(loggerSpy).toHaveBeenCalledWith('Incoming-data: Ethereum address');
                expect(actionSheetSpy).toHaveBeenCalledWith({
                    data: {
                        data: element,
        ...
        it('Should handle ETH URI with amount (value)', () => {
            ...
            data.forEach(element => {
                ...
                expect(incomingDataProvider.redir(element.uri, { activePage: 'ScanPage' })).toBe(true);
        ...
        it('Should handle Bitcoin URI', () => {
            let data = [
                ...
                'bitcoin:1A1zP1eP5QGefi2DMPTfTL5SLmv7DivfNa?amount=1.0000',
        ...
        it('Should Handle Bitcoin Cash URI with legacy address', fakeAsync(() => {
            ...
            expect(incomingDataProvider.redir(data, { activePage: 'ScanPage' })).toBe(true);
            expect(loggerSpy).toHaveBeenCalledWith('Incoming-data: Bitcoin Cash URI with legacy address');
        ...
        it('Should handle any type of address/coin using BitPay URI', fakeAsync(() => {
            ...
            data.forEach(element => {
                ...
                let params = new URLSearchParams(element.replace(`bitpay:${address}`, ''));
                const message = params.get('message');
                const coin = Coin[params.get('coin').toUpperCase()];
                const { unitToSatoshi } = currencyProvider.getPrecision(coin);
                const amount = parseInt((Number(params.get('amount')) * unitToSatoshi).toFixed(0), 10).toString();
        ...
        it('Should handle Bitcoin Livenet and Testnet Plain Address', () => {
            ...
            data.forEach(element => {
                expect(incomingDataProvider.redir(element, { activePage: 'ScanPage' })).toBe(true);
                expect(loggerSpy).toHaveBeenCalledWith('Incoming-data: Bitcoin plain address');
                expect(actionSheetSpy).toHaveBeenCalledWith({
                    data: {
                        data: element,
                        type: 'bitcoinAddress',
        ...
        it('Should handle private keys', () => {
            let data = [
                '6PnSQd4UamkL5LDZrAsmymQrAgj1jywES6frfp5DeFGWni7VouwjxeJ68z',
        ...
        xit('Should handle Omega Card URI', () => {
            ...
            expect(incomingDataProvider.redir(data, { activePage: 'ScanPage' })).toBe(true);
            expect(loggerSpy).toHaveBeenCalledWith('Incoming-data (redirect): Omega Card URL');
            expect(eventsSpy).toHaveBeenCalledWith('IncomingDataRedir', nextView);
        });
        it('Should handle BitPay redir link', () => {
            ...
            data.forEach(link => {
                ...
                expect(loggerSpy).toHaveBeenCalledWith('Incoming-data (redirect): BitPay Redir');
    ...
    describe('Function: parseData', () => {
        ...
        it('Should return the correct type for each kind of data', () => {
            const dataArray = [
                ...
                {
                    data: 'bitcoin:1A1zP1eP5QGefi2DMPTfTL5SLmv7DivfNa?message=test%20message',
                    expectedType: 'BitcoinUri'
                },
                {
                ...
                {
                    data: 'bitcoin:1A1zP1eP5QGefi2DMPTfTL5SLmv7DivfNa?amount=1.0000&label=Genesis%20Bitcoin%20Address&message=test%20message',
                    expectedType: 'BitcoinUri'
                },
                {
                ...
                {
                    data: 'BITCOINCASH:QZCY06MXSK7HW0RU4KZWTRKXDS6VF8Y34VRM5SF9Z7?amount=1.00000000',
                    expectedType: 'BitcoinCashUri'
                },
                {
                    data: 'bchtest:pzpaleegjrc0cffrmh3nf43lt3e3gu8awqyxxjuew3?amount=12.00000000',
                    expectedType: 'BitcoinCashUri'
                },
                {
                ...
                {
                    data: 'ethereum:0xb506c911deE6379e3d4c4d0F4A429a70523960Fd?value=1543000000000000000',
                    expectedType: 'EthereumUri'
                },
                {
                    data: 'ethereum:0xb506c911deE6379e3d4c4d0F4A429a70523960Fd?value=1543000000000000000?gasPrice=0000400000000000000',
                    expectedType: 'EthereumUri'
                },
                {
                ...
                {
                    data: 'bitpay:mrNYDWy8ZgmgXVKDb4MM71LmfZWBwGztUK?coin=btc&amount=0.0002&message=message',
                    expectedType: 'BitPayUri'
                },
                {
                    data: 'bitpay:1HZJoc4ZKMvyAYcYCU1vbmwm3KzZq34EmU?coin=btc&amount=0.0002&message=asd',
                    expectedType: 'BitPayUri'
                },
                {
                    data: 'bitpay:0xDF5C0dd7656bB976aD7285a3Fb80C0F6B9604576?coin=eth&amount=1543000000000000000?gasPrice=0000400000000000000',
                    expectedType: 'BitPayUri'
                },
                {
                    data: 'bitpay:bchtest:qp2gujqu2dsp6zs4kp0pevm2yl8ydx723q2kvfn7tc?coin=bch&amount=0.0002&message=asd',
                    expectedType: 'BitPayUri'
                },
                {
                    data: 'bitpay:bitcoincash:qpcc9qe5ja73k7ekkqrnjfp9tya0r3d5tvpm2yfa0d?coin=bch&amount=0.0002&message=asd',
                    expectedType: 'BitPayUri'
                },
                {
                ...
                {
                    data: 'copay:RTpopkn5KBnkxuT7x4ummDKx3Lu1LvbntddBC4ssDgaqP7DkojT8ccxaFQEXY4f3huFyMewhHZLbtc',
                    expectedType: 'JoinWallet'
                },
                {
                    data: 'RTpopkn5KBnkxuT7x4ummDKx3Lu1LvbntddBC4ssDgaqP7DkojT8ccxaFQEXY4f3huFyMewhHZLbtc',
                    expectedType: 'JoinWallet'
                },
                {
                    data: '6PnSQd4UamkL5LDZrAsmymQrAgj1jywES6frfp5DeFGWni7VouwjxeJ68z',
                    expectedType: 'PrivateKey'
                },
                {
                    data: '5Hwgr3u458GLafKBgxtssHSPqJnYoGrSzgQsPwLFhLNYskDPyyA',
                    expectedType: 'PrivateKey'
                },
                {
                    data: 'L1aW4aubDFB7yfras2S1mN3bqg9nwySY8nkoLmJebSLD5BWv3ENZ',
                    expectedType: 'PrivateKey'
                },
                {
                    data: "1|sick arch glare wheat anchor innocent garbage tape raccoon already obey ability|testnet|m/44'/1'/0'|false",
                ...
                {
                    data: '1|sick arch glare wheat anchor innocent garbage tape raccoon already obey ability|null|null|false|null',
                    expectedType: 'ImportPrivateKey'
                }
            ];
...
//# sourceMappingURL=incoming-data.spec.js.map