...
describe('TxConfirmNotificationProvider', () => {
    let txConfirmNotificationProvider;
    ...
    describe('checkIfEnabled function', () => {
        it('should reject an error if getTxConfirmation function fails', () => {
            spyOn(persistenceProvider, 'getTxConfirmNotification').and.returnValue(Promise.reject('Error'));
            txConfirmNotificationProvider
                .checkIfEnabled('txidTest')
                .catch(err => expect(err).toBeDefined());
        });
        it('should return value if getTxConfirmation function works as expected', () => {
            spyOn(persistenceProvider, 'getTxConfirmNotification').and.returnValue(Promise.resolve());
            txConfirmNotificationProvider
                .checkIfEnabled('txidTest')
                .then(res => {
    ...
    describe('subscribe function', () => {
        it('should try to subscribe and show logger error if something is wrong', () => {
            ...
            spyOn(persistenceProvider, 'setTxConfirmNotification').and.returnValue(Promise.reject('Error'));
            txConfirmNotificationProvider.subscribe(walletMock, { txid: 'txid1' });
            expect(loggerSpy).toHaveBeenCalled();
        });
        it('should subscribe a client', () => {
            const setTxConfirmNotificationSpy = spyOn(persistenceProvider, 'setTxConfirmNotification').and.returnValue(Promise.resolve());
            txConfirmNotificationProvider.subscribe(walletMock, { txid: 'txid1' });
    ...
    describe('unsubscribe function', () => {
        it('should try to subscribe and show logger error if something is wrong', () => {
            const loggerSpy = spyOn(logger, 'error');
            spyOn(persistenceProvider, 'removeTxConfirmNotification').and.returnValue(Promise.reject('Error'));
            txConfirmNotificationProvider.unsubscribe(walletMock, 'txid1');
            expect(loggerSpy).toHaveBeenCalled();
        });
        it('should unsubscribe a client', () => {
            const removeTxConfirmNotificationSpy = spyOn(persistenceProvider, 'removeTxConfirmNotification').and.returnValue(Promise.resolve());
            txConfirmNotificationProvider.unsubscribe(walletMock, 'txid1');
            expect(removeTxConfirmNotificationSpy).toHaveBeenCalled();
        });
    });
});
//# sourceMappingURL=tx-confirm-notification.spec.js.map