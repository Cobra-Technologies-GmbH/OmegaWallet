...
let BitPayAccountProvider = class BitPayAccountProvider {
            ...
            this.bitPayProvider.postAuth(json, data => {
                ...
                this.fetchBasicInfo(apiContext, (err, basicInfo) => {
                    ...
                    if (pairingReason) {
                        ...
                        msg = this.replaceParametersProvider.replace(this.translate.instant('To {{reason}} you must first add your Omega Account - {{email}}'), { reason, email });
                    }
                    else {
                        let email = pairData.email;
                        msg = this.replaceParametersProvider.replace(this.translate.instant('Add this Omega account ({{email}})?'), { email });
                    }
    ...
    checkOtp(pairData, cb) {
        if (pairData.otp) {
            let msg = this.translate.instant('Enter Two Factor for your Omega Account');
