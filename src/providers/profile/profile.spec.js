...
describe('Profile Provider', () => {
    ...
    const walletClientMock = {
        ...
        importFromExtendedPrivateKey: (_xPrivKey, _opts, _cb) => {
            return _cb(null, walletToImport);
        },
        importFromExtendedPublicKey: (_xPubKey, _externalSource, _entropySource, _opts, _cb) => {
            return _cb(null, walletToImport);
        },
        importFromMnemonic: (_words, _opts, _cb) => {
            return _cb(null, walletToImport);
        },
        ...
        seedFromExtendedPublicKey: (_extendedPublicKey, _externalSource, _entropySource, _opts) => {
            return true;
        },
        createWallet: (_name, _myName, _m, _n, _opts, _cb) => {
            return _cb(null);
        },
        joinWallet: (_secret, _myName, _opts, _cb) => {
            return _cb(null);
        },
        setNotificationsInterval: _updatePeriod => {
            return true;
    ...
    describe('setBackupGroupFlag', () => {
        ...
        it('should return if !keyId', () => {
            keyId = undefined;
            const setBackupGroupFlagSpy = spyOn(persistenceProvider, 'setBackupGroupFlag');
            profileProvider.setBackupGroupFlag(keyId);
    ...
    describe('setFastRefresh', () => {
        it('should set needsBackup to false for a specified walletId', () => {
            const setNotificationsIntervalSpy = spyOn(profileProvider.wallet.id1, 'setNotificationsInterval');
    ...
    describe('Function: normalizeMnemonic', () => {
        ...
        it('Should return the same words list if it is already normalized', () => {
            ...
            expect(profileProvider.normalizeMnemonic(words)).toEqual('mom mom mom mom mom mom mom mom mom mom mom mom');
        });
        ...
        it('Should return words list normalized for all different languages if it is called with capital letters and spaces in different combinations of positions', () => {
            const words = '     mom  Mom mom           mom mom MOM mom moM mom mom mom mom    ';
            expect(profileProvider.normalizeMnemonic(words)).toEqual('mom mom mom mom mom mom mom mom mom mom mom mom');
            const spanishWords = ' tener golpe máquina   cumbre caÑón UNO lino Vigor RÁbano sombra oleada multa  ';
            expect(profileProvider.normalizeMnemonic(spanishWords)).toEqual('tener golpe máquina cumbre cañón uno lino vigor rábano sombra oleada multa');
            const frenchWords = '  effacer embryon groupe   rigide BUSTIER caresser adjectif colonel friable bolide terrible divertir  ';
            expect(profileProvider.normalizeMnemonic(frenchWords)).toEqual('effacer embryon groupe rigide bustier caresser adjectif colonel friable bolide terrible divertir');
            const italianWords = 'AVERE   farfalla siccome balzano grinza dire baGnato fegato nomina satollo baldo nobile  ';
            expect(profileProvider.normalizeMnemonic(italianWords)).toEqual('avere farfalla siccome balzano grinza dire bagnato fegato nomina satollo baldo nobile');
            const dutchWords = 'blush cube farm element maTH gauge defy install garden awkward wide fancy  ';
            expect(profileProvider.normalizeMnemonic(dutchWords)).toEqual('blush cube farm element math gauge defy install garden awkward wide fancy');
            const polishWords = ' spider  rose radio defense   garment voice kitten dune    license chunk   glove shuffle';
            expect(profileProvider.normalizeMnemonic(polishWords)).toEqual('spider rose radio defense garment voice kitten dune license chunk glove shuffle');
            const germanWords = 'Harsh Original Stove Fortune Enforce Young Throw Clay Liberty Certain Loud Aware';
            expect(profileProvider.normalizeMnemonic(germanWords)).toEqual('harsh original stove fortune enforce young throw clay liberty certain loud aware');
            const japaneseWords = '  のぼる　しゅみ　ぜっく　おおどおり　 そんしつ　はさん　けつえき　くうき　こんき　ひやす　うよく　しらせる   ';
            expect(profileProvider.normalizeMnemonic(japaneseWords)).toEqual('のぼる　しゅみ　ぜっく　おおどおり　そんしつ　はさん　けつえき　くうき　こんき　ひやす　うよく　しらせる');
            const simplifiedChineseWords = '  泰 柱 腹 侵 米 强 隙 学 良  迅 使 毕 ';
            expect(profileProvider.normalizeMnemonic(simplifiedChineseWords)).toEqual('泰 柱 腹 侵 米 强 隙 学 良 迅 使 毕');
            const traditionalChineseWords = ' 只 沒 結 解 問 意   建 月  公 無 系 軍 ';
            expect(profileProvider.normalizeMnemonic(traditionalChineseWords)).toEqual('只 沒 結 解 問 意 建 月 公 無 系 軍');
            const russianWords = 'proud admit  enforce  fruit  prosper  odor approve present have there smart kitten ';
            expect(profileProvider.normalizeMnemonic(russianWords)).toEqual('proud admit enforce fruit prosper odor approve present have there smart kitten');
            const portugueseWords = 'ABSENT POND DEPOSIT SMOOTH EMPTY TROPHY LOUD THERE ADMIT WHISPER MULE MORE';
            expect(profileProvider.normalizeMnemonic(portugueseWords)).toEqual('absent pond deposit smooth empty trophy loud there admit whisper mule more');
        });
    });
    ...
    describe('loadAndBindProfile', () => {
        ...
        beforeEach(() => {
            ...
            storeProfileSpy = spyOn(persistenceProvider, 'storeProfile').and.returnValue(Promise.resolve());
        ...
        it('should get and bind profile with migrated credentials', () => {
            BwcProviderMock.prototype.upgradeMultipleCredentialsV1 = (_oldCredentials) => {
                const migrated = {
                    credentials: [
                        {
                            walletId: 'id1',
    ...
    describe('isDisclaimerAccepted', () => {
        ...
        it('should set disclaimerAccepted with true if OLD flag is true', () => {
            ...
            spyOn(persistenceProvider, 'getCopayDisclaimerFlag').and.returnValue(Promise.resolve(true));
            profileProvider
                .isDisclaimerAccepted()
                .then(() => {
        ...
        it('should not set disclaimerAccepted if OLD flag is not present', () => {
            profileProvider.profile.disclaimerAccepted = false;
            spyOn(persistenceProvider, 'getCopayDisclaimerFlag').and.returnValue(Promise.resolve(null));
    ...
    describe('createWallet', () => {
        ...
        beforeEach(() => {
            ...
            spyOn(actionSheetProvider, 'createEncryptPasswordComponent').and.returnValue({
                present: () => { },
                dismiss: () => { }
            });
        });
    ...
    describe('joinWallet', () => {
        beforeEach(() => {
            spyOn(profileProvider, 'askToEncryptKey').and.returnValue(Promise.resolve(true));
            const opts = {
                pushNotifications: { enabled: true },
                bwsFor: 'id1'
            };
        ...
        it('should join wallet and publish "Local/WalletUpdate" event', () => __awaiter(void 0, void 0, void 0, function* () {
            const opts = {
                secret: 'secret5',
                coin: 'btc',
                myName: 'Gabriel M'
        ...
        it('should fails to join wallet if you already joined that wallet', () => __awaiter(void 0, void 0, void 0, function* () {
            const opts = {
                secret: 'secret1',
                coin: 'btc',
                myName: 'Gabriel M'
    ...
    describe('deleteWalletClient', () => {
        ...
        it('should delete wallet client', () => __awaiter(void 0, void 0, void 0, function* () {
            const wallet = {
                credentials: {
                    walletId: 'id1'
                },
    ...
    describe('setLastKnownBalance', () => {
        beforeEach(() => {
            spyOn(persistenceProvider, 'getLastKnownBalance').and.callFake((_id) => {
    ...
    describe('Desktop notifications', () => {
        ...
        it('should call showDesktopNotifications and go through NewCopayer path for MacOS', () => __awaiter(void 0, void 0, void 0, function* () {
            spyOn(platformProvider, 'getOS').and.returnValue({
                OSName: 'MacOS'
            });
            onEventNotificationType = 'NewCopayer';
            const replaceSpy = spyOn(replaceParametersProvider, 'replace').and.returnValue('body1');
            // Using createWallet just to test showDesktopNotifications private function
            yield profileProvider.createWallet(opts).catch(err => {
                expect(err).not.toBeDefined();
            });
            expect(replaceSpy).toHaveBeenCalledTimes(1);
        }));
        it('should call showDesktopNotifications and go through NewCopayer path for other OS', () => __awaiter(void 0, void 0, void 0, function* () {
            spyOn(platformProvider, 'getOS').and.returnValue({
                OSName: 'Linux'
            });
            onEventNotificationType = 'NewCopayer';
        ...
        it('should call showDesktopNotifications and go through WalletComplete path', () => __awaiter(void 0, void 0, void 0, function* () {
        ...
        it('should call showDesktopNotifications and go through NewTxProposal path', () => __awaiter(void 0, void 0, void 0, function* () {
            ...
            spyOn(BwcProviderMock.prototype, 'getClient').and.returnValue(newWalletClient);
        ...
        it('should call showDesktopNotifications and go through NewIncomingTx path', () => __awaiter(void 0, void 0, void 0, function* () {
        ...
        it('should call showDesktopNotifications and go through TxProposalFinallyRejected path', () => __awaiter(void 0, void 0, void 0, function* () {
        ...
        it('should call showDesktopNotifications and go through TxConfirmation path', () => __awaiter(void 0, void 0, void 0, function* () {
