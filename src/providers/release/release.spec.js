...
describe('Release Provider', () => {
    ...
    it('should get successfully the latest app version', () => {
        releaseProvider.getLatestAppVersion().then((data) => {
            const version = data.version;
            expect(version).toBeDefined();
            expect(version).toEqual(latestAppVersion);
        });
        const bwsReq = httpMock.expectOne('https://bws.omega.eco/bws/api/latest-version');
    ...
    it('should check unsuccessfully the latest app version format', () => {
        ...
        expect(loggerSpy).toHaveBeenCalledWith('Cannot verify the format of version tag. latestVersion V..3.3.3 - currentVersion v1.1.1');
    });
    it('should check unsuccessfully the current app version format', () => {
        ...
        expect(result).toBeFalsy;
        expect(loggerSpy).toHaveBeenCalledWith('Cannot verify the format of version tag. latestVersion v2.2.2 - currentVersion V..1.1.1');
    });
